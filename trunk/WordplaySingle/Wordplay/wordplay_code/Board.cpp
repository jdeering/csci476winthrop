...
int Board::submitWord()
{
	...
	if (isWord())
	{
		...
		//let the user know it was submitted
		GameFramework::readText("You just submitted the word " + returnWord() + ".");

		//remove & replace the letters we just used
		replaceLetters();
	}
	else{
		//if it's not, you'll return 0
		//let the user know
		GameFramework::readText("I am sorry, " + returnWord() + "is not in the dictionary.");
	}
