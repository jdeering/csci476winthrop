	...
	if (bytesRead != 0)
	{
		...
		if (bytesAvail > MAX_MESSAGE_SIZE)
		{
			while (bytesRead >= MAX_MESSAGE_SIZE)
			{
				...
				ReadFile(hStdIn_Parent, incMessage, MAX_MESSAGE_SIZE, &bytesRead, NULL);  //read the stdout pipe
				msgStream.write(incMessage, bytesRead); // copy message to stringstream
	...
	if(mouse.StateChange(button, state, x, y))
	{
		...
		if(x >= 0 && x <= 600 && y >= 0 && y <= 600/* && gameRunning*/) // Game event
		{
			...
			{				
				message.clear();
				message.str("");
				message << "103 " << button << " " << state << " " << sprite_name.c_str() << std::endl;
				mouse_message = message.str();
				sendMessage(mouse_message.c_str());
				message.clear();
				mouse_message.clear();
...
void Framework::UpdateKeyboard()
{
	...
	while(keyboard.StateChange(key, state))
	{
		...
		message.clear();
		message.str("");
		message << "201 " << key << " " << state << std::endl;
