	...
	if (bytesRead != 0)
	{
		...
		if (bytesAvail > GFW_BUFFER_SIZE)
		{
			while (bytesRead >= GFW_BUFFER_SIZE)
			{
				...
				msgStream.write(_msgBuffer, bytesRead); // copy message to std::stringstream
...
void GameFramework::kybdMessage(const char *message)
{
	int p1, p2;
	if (!cb_KH) return;
...
void GameFramework::mouseMessage(const char *message)
{
	int opcode;
	int mouseX, mouseY;
	int p1, p2, p3, p4;
...
GFSprite& GameFramework::createSprite(std::string aname, int x, int y, int w, int h)
{
	...
	ENGINE->CreateSprite(_msgBuffer);
	_clrBuffer();
...
GFSprite& GameFramework::createSprite(std::string aname, int x, int y)
{
	...
	//sendMessage();
	/* CALL ENGINE FUNCTION */
	ENGINE->CreateSpriteRefDimensions(_msgBuffer);
	_clrBuffer();
...
GFText& GameFramework::createTextFromAsset(std::string aname, int x, int y)
{
	...
	//sendMessage();
	/* CALL ENGINE FUNCTION */
	ENGINE->CreateTextFromRef(_msgBuffer);
	_clrBuffer();
...
GFText& GameFramework::createTextFromString(std::string str, int x, int y)
{
	...
	//sendMessage();
	/* CALL ENGINE FUNCTION */
	ENGINE->CreateTextFromString(_msgBuffer);
	_clrBuffer();
...
void GameFramework::setTextBGColor(GFText &t, int r, int g, int b)
{
	...
	if (_index_table[t._ref] == INDEX_TAKEN)
	{
		...
		//sendMessage();
		/* CALL ENGINE FUNCTION */
		ENGINE->SetTextBackgroundColor(_msgBuffer);
		_clrBuffer();
	}
};
...
void GameFramework::postScore()
{	
	/* CREATE THE MESSAGE BUFFER TO SEND THE DATA TO THE FRAMEWORK */
	sprintf(_msgBuffer, "%d %d", POST_SCORE, score);

	/* SEND THE MESSAGE TO THE FRAMEWORK */
	//sendMessage();
	/* CALL ENGINE FUNCTION */
	ENGINE->PostScore(_msgBuffer);
	_clrBuffer();
}
...
HANDLE GameFramework::stderrFW = GetStdHandle(STD_ERROR_HANDLE);
...
Framework* GameFramework::ENGINE = NULL;