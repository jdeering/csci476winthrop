...
// MARKUP_MBCS ANSI/double-byte std::strings on Windows
// MARKUP_STL (default except VC++) use STL std::strings instead of MFC std::strings
// MARKUP_SAFESTR to use std::string _s functions in VC++ 2005 (_MSC_VER >= 1400)
...
#if _MSC_VER > 1000 // VC++
#pragma once
#if ! defined(MARKUP_SAFESTR) // not VC++ safe std::strings
#pragma warning(disable:4996) // VC++ 2005 deprecated function warnings
#endif // not VC++ safe std::strings
...
#if defined(MARKUP_SAFESTR) // VC++ safe std::strings
...
// std::string type and function defines (compiler and build-option dependent)
// Define MARKUP_STL to use STL std::strings
//
...
// Allow function args to accept std::string objects as constant std::string pointers
struct MCD_CSTR
{
	MCD_CSTR() { pcsz=NULL; };
...
protected:
	...
	struct TokenPos
	{
		...
		{
			// std::string compare ignore case
			bool bNonA = false;
			MCD_CHAR c1, c2;
			while ( n-- )
			{
