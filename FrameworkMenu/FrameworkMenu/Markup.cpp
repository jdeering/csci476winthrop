...
#include <mbstd::string.h> // for VC++ _mbclen
#endif // WINCONV
...
// Customization
#define x_EOL MCD_T("\r\n") // can be \r\n or \n or empty
#define x_EOLLEN (sizeof(x_EOL)/sizeof(MCD_CHAR)-1) // std::string length of x_EOL
...
int CMarkup::UTF16To8( char* pszUTF8, const unsigned short* pwszUTF16, int nUTF8Count )
{
	...
	// the pwszUTF16 source must be a NULL-terminated UTF-16 std::string
	// if pszUTF8 is NULL, the number of bytes required is returned and nUTF8Count is ignored
	// otherwise pszUTF8 is filled with the result std::string and NULL-terminated if nUTF8Count allows
...
int CMarkup::UTF8To16( unsigned short* pwszUTF16, const char* pszUTF8, int nUTF8Count )
{
	...
	// the pszUTF8 source must be a UTF-8 std::string which will be processed up to NULL-terminator or nUTF8Count
	...
	// if pwszUTF16 is not NULL it is filled with the result std::string and it must be large enough
...
{
	// The token contains indexes into the document identifying a small substd::string
	// Build the substd::string from those indexes and return it
...
MCD_STR CMarkup::x_GetTagName( int iPos ) const
{
	...
	// Return substd::string of document
	return x_GetToken( token );
}
...
bool CMarkup::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, MCD_PCSZ pValue, int nFlags /*=0*/ )
{
	...
	{
		// Insert std::string name value pair
		MCD_STR strFormat;
		strFormat = MCD_T(" ");
		strFormat += pAttrib;
		strFormat += MCD_T("=");
...
MCD_STR CMarkup::x_GetData( int iPos ) const
{
	...
	// Return a std::string representing data between start and end tag
	// Return empty std::string if there are any children elements
	MCD_STR strData;
...
void CMarkup::x_DocChange( int nLeft, int nReplace, const MCD_STR& strInsert )
{
	...
	// When creating a document, reduce reallocs by reserving std::string space
...
int CMarkup::x_IConv( ConvertEncoding& convert, void* pTo, int nToCharSize, int nFromCharSize )
{
	...
	if ( cd != (iconv_t)-1 )
	{
		...
		while ( nFromLenRemaining )
		{
			...
			if ( nResult == (size_t)-1 )
			{
				// Bypass bad char, question mark denotes problem in source std::string
...
bool CMarkup::x_ReadText( MCD_STR& strDoc, FilePos& file )
{
	...
	else // single or multibyte file (i.e. not UTF-16)
	{
		...
		else // load directly into std::string
		{
...
bool CMarkup::x_WriteText( const MCD_STR& strDoc, FilePos& file )
{
	...
	else // single or multibyte file (i.e. not UTF-16)
	{
		...
		else // save directly from std::string
		{
			file.nFileTextLen = nWriteStrLen;
			bSuccess = x_Write( NULL, file, pDoc );
		}
